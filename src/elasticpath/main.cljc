(ns elasticpath.main
  "Kotodama WASM entrypoint for the Elasticpath clean-room actor (L4) — Clojure port.
  ...
  facts (`elasticpath.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "elasticpath")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"elasticpath.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "elasticpath-compat" :tier tier :entities entities} 200])
...
(defn start! [] :elasticpath-compat/ready)
